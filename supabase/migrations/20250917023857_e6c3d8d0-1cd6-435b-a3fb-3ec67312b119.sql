...
CREATE POLICY "Walkers can view dogs for accepted requests" ON public.dogs FOR SELECT USING (id IN (
  SELECT dog_id FROM public.walk_requests WHERE walker_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid())
));

-- RLS Policies for walk_requests
DROP POLICY IF EXISTS "Users can view their own requests" ON public.walk_requests;
CREATE POLICY "Users can view their own requests" ON public.walk_requests FOR SELECT USING (
...
CREATE POLICY "Users can update their own requests" ON public.walk_requests FOR UPDATE USING (
  owner_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid()) OR
  walker_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid())
);
...
CREATE POLICY "Users can create reviews for completed walks" ON public.reviews FOR INSERT WITH CHECK (
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'chat_messages' AND column_name = 'walk_request_id') THEN
    ...
    CREATE POLICY "Users can view messages for their requests" ON public.chat_messages FOR SELECT USING (
      walk_request_id IN (SELECT id FROM public.walk_requests WHERE owner_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid()) OR walker_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid())));
    CREATE POLICY "Users can send messages for their requests" ON public.chat_messages FOR INSERT WITH CHECK (
      sender_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid()) AND walk_request_id IN (SELECT id FROM public.walk_requests WHERE owner_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid()) OR walker_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid())));
  END IF;
END $$;
