...
-- RLS Policies for walk_requests table
...
-- RLS Policies for walk_sessions table
...
-- RLS Policies for geo_points table
...
BEGIN
    ...
    SET rating = (
        ...
        FROM reviews 
        WHERE reviewed_id = walker_profiles.user_id
    ),
    total_walks = (
        SELECT COUNT(*)
        FROM walk_requests 
        WHERE walker_id = walker_profiles.user_id 
...
-- Trigger to update walker rating when a review is inserted
